...
def _process_row(
    ...
    for question_code in ("Q9", "Q10", "Q11", "Q12", "Q13", "Q14", "Q15"):
        ...
        for item in question_config["itens"]:
            ...
            val = values_by_code.get(item_code, "")
            if item_code in ("Q14_SAG_4", "Q15_ITI_1", "Q15_ITI_2", "Q15_ITI_3") and val:
                norm = normalize_label(val)
                if norm == "nunca":
                    val = "Sempre"
                elif norm == "raramente":
                    val = "Frequentemente"
                elif norm in ("as vezes", "a vezes"):
                    val = "As vezes"
                elif norm == "frequentemente":
                    val = "Raramente"
                elif norm == "sempre":
                    val = "Nunca"
            responses_by_item[item_code] = val
